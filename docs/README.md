...
> **Règle d'or :** la documentation active dans `docs/` (et ses sous-dossiers `audit/`, `commissaire_aux_apports/`) est **prioritaire** sur les documents archivés (`docs/archive/`). Aucun document archivé ne doit être utilisé comme référence opérationnelle courante.
...
|---|---|
| Nouvel arrivant développeur | [`HANDOVER_DEVELOPPEUR.md`](./HANDOVER_DEVELOPPEUR.md), puis [`DOCUMENTATION_TECHNIQUE.md`](./DOCUMENTATION_TECHNIQUE.md), puis [`DICA_ORCHESTRATOR_GUIDE.md`](./DICA_ORCHESTRATOR_GUIDE.md). |
| Utilisateur final | [`MODE_EMPLOI.md`](./MODE_EMPLOI.md) puis [`GUIDE_UTILISATEUR.md`](./GUIDE_UTILISATEUR.md). |
| Administrateur | [`GUIDE_ADMINISTRATEUR.md`](./GUIDE_ADMINISTRATEUR.md). |
| Ops / déploiement | [`GUIDE_DEPLOIEMENT.md`](./GUIDE_DEPLOIEMENT.md), [`CHECKLIST_SMOKE_KILLSWITCH.md`](./CHECKLIST_SMOKE_KILLSWITCH.md), `../.github/workflows/README.md`. |
...
> Documents internes de pilotage et de valorisation. Non destinés à un usage marketing ; à manipuler avec discernement.
...
Documents qui contredisent l'état actuel du code (services supprimés, version incompatible).
...
Documents anciens utiles pour la traçabilité, l'audit et la valorisation. Ne pas opposer à un état présent.
...
Chaque document archivé porte en tête un encadré `⚠️ DOCUMENT ARCHIVÉ` indiquant son statut, sa date d'archivage, la raison et le document de remplacement.
...
1. **Toute nouvelle documentation est créée dans `docs/`** (ou un sous-dossier dédié si elle relève d'audit, de commissaire aux apports, etc.). Elle est référencée dans le présent index.
2. **Tout document devenu obsolète est archivé**, pas supprimé : déplacer dans `docs/archive/obsolete/` ou `docs/archive/historical/`, ajouter un encadré `⚠️ DOCUMENT ARCHIVÉ` en tête, mettre à jour le présent index et le `README.md` racine.
3. **Toute référence à un module supprimé doit être marquée comme telle** (par exemple : `*(archivé YYYY-MM-DD, service remplacé par X)*`).
4. **Les chiffres opérationnels** (nombre de tests, nombre de suites, vulnérabilités, etc.) sont datés et reproductibles via les commandes `npm run lint` / `npm run test:run` / `npm audit`. Ne pas mettre de chiffres non sourcés.
5. **La distinction documentation technique / commerciale / audit / historique est stricte.** Les supports commerciaux n'ont pas vocation à être opposables comme référence technique.
6. **Le `README.md` racine et le présent `docs/README.md` sont les deux uniques points d'entrée fiables.** Toute autre porte d'entrée informelle doit y renvoyer.
