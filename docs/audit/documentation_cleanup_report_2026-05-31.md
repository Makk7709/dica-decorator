...
| `lovable-tagger` cité comme dépendance dans certains documents ; **retiré du runtime et des devDependencies**. | Cohérence vérifiée : pas de référence active dans la documentation post-cleanup. | Statut clarifié dans `README.md` racine § 3 et déjà documenté dans `docs/AUDIT_DEPENDANCES.md`. |
